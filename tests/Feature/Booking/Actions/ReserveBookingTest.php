...
it('crée une réservation en attente de paiement avec blocage temporaire', function () {
    $trip = Trip::factory()->create([
        'capacity' => 30,
    ]);

    $luggage = Luggage::factory()
        ->for($this->expediteur)
        ->create([
    ...
    $booking = app(ReserveBooking::class)->execute($payload);
...
it('rejette une valise déjà réservée', function () {
    ...
    app(ReserveBooking::class)->execute($payload);
});
